        ...
        function(callback) {
            boundConfigModel
                ...
                .then(async function(row) {
                    var qtdItbi = await itbiDAO.count()
                    if (qtdItbi > 0) {
...
function migrate(next) {
    ...
    async.whilst(
        ...
        async function(done) {
            ...
            var list;
            if (count === 0) {
                list = await itbiDAO.fillList('NU_COD_BAIRRO IS NULL', 'TecGeo.IMOBILIARIO', 'NU_SEQUENCIAL');
            } else {
                list = await itbiDAO.fillList('NU_COD_BAIRRO = \'' + codBairro + '\'', 'TecGeo.IMOBILIARIO', 'NU_SEQUENCIAL');
            }
            var handlingStart;

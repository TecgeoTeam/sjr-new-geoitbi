...
function(
  ...
  return declare([BaseWidget], {
    ...
    fillTransmissaoTable: function(tableFiltroTransmissao, result) {
      ...
      for (let i = result.length - 1; i >= 0; i--) {
        if (result[i].DE_USO === null){
          result[i].DE_USO = $("#tipoImovelTransmissaoInput").val();
        }
      }
      ...
      trTh.cells[0].appendChild(document.createTextNode('Geocódigo'));
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        trConteudo.cells[0].appendChild(document.createTextNode(result[i]['DE_GEOCODE_LOTE']));
        trConteudo.cells[1].appendChild(document.createTextNode(result[i]['ID']));
        trConteudo.cells[2].appendChild(document.createTextNode(result[i]['DE_USO']));
        trConteudo.cells[3].appendChild(document.createTextNode(result[i]['DE_TIPO_IMOVEL']));
        trConteudo.cells[4].appendChild(document.createTextNode(result[i]['DE_PADRAO_CONSTRUCAO']));
        trConteudo.cells[5].appendChild(document.createTextNode(result[i]['DE_IDADE']));
        trConteudo.cells[6].appendChild(document.createTextNode(result[i]['NU_AREA_CONSTRUIDA']));
        trConteudo.cells[7].appendChild(document.createTextNode(result[i]['DE_CONSERVACAO']));
    ...
    addParametroTransmissao: function() {
      ...
      if (elem.value === 'Geocódigo da Unidade') {
      ...
      if (geocodeStm) {
        ...
        baseObject.name = 'DE_GEOCODE_LOTE';
      }
      if (idTransmissao) {
    ...
    searchTransmissao: function(dataInicio, dataFim, parametrosTransmissao, callback) {
      ...
      for (let i = 0; i < parametrosTransmissao.length; i++) {
        ...
        /*if (parametrosTransmissao[i].name === 'ID') {
          whereTransmissao.push({
      ...
      console.log("WHERE TRANSMISSAO AND UNIDADE",data);
     var settings = {
          "async": true,
          "crossDomain": true,
          "url": "http://107.180.106.82:6010/geoportal/transmissoes",
          "method": "POST",
          "headers": {
      ...
      $.ajax(settings).then(function(resultTrans, textStatus, jqXHR) {
        ...
        //TODO FAZER UM TABELÃO PRA TRANSMISSOES
        // COM AS CARACTERISTICAS DA UNIDADE, PRA FACILITAR NO BACKEND
        console.log(resultTrans);
        callback(resultTrans);
      }, function(jqXHR, textStatus, errorThrown) {
        // When AJAX call has faile
      ...
      that.searchPrioritarioLoteTransmissao(geocodeLotes, function(resultPrioritario) {
        ...
        that.searchNaoPrioritarioLoteTransmissao(geocodeLotes, function(resultNaoPrioritario) {
          ...
          for (let j = 0; j < result.features.length; j++) {
            if (result.features[j].attributes['sjr_cadastro.SDE.Lote.stg_geocode']) {
              geocodeObject[result.features[j].attributes['sjr_cadastro.SDE.Lote.stg_geocode']] = result.features[j];
    ...
    searchPrioritarioLoteTransmissao: function(geocodeLotes, callback) {
      ...
      query.outFields = ['sjr_cadastro.SDE.Lote.OBJECTID', 'sjr_cadastro.SDE.Lote.stg_geocode'];
      query.outSpatialReference = {
        wkid: 3857
      };
      query.where = 'sjr_cadastro.SDE.Lote.stg_geocode IN(';
...
window.selectFeatureTrans = function(geocode, divId, features) {
  ...
  if (feature) {
    ...
    //DocumentUtil.showMessage('info', 'Esta transmissão não possui representação no mapa!', true, 3000, null);
    alert("Esta transmissão não possui representação no mapa!")
  }

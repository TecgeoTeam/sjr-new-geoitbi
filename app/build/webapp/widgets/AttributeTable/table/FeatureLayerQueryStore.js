define([
  ...
  let FeatureLayerQueryStore = declare(null, {
    ...
    query: function(query, options) {
      ...
      let start = (options && options.start) || 0;
      let count = /* options.count || */ this.batchCount;
      let filterIds = null;
      ...
      if (this.objectIds) {
        ...
        filterIds = filterIds || this.objectIds;
        ...
        if (sort && sort.length > 0) {
          let orderByFields = array.map(sort, function(s) {
      ...
      let invalidIds = !(queryObj.objectIds && queryObj.objectIds.length > 0);
      let invalidWhereStr = !queryObj.where;
      ...
      results.total = results.then(lang.hitch(this, function(result) {
        ...
        let objectIdFieldName = this.layer.objectIdField;

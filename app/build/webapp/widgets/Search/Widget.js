...
function(declare, lang, array, html, when, on, aspect, query, keys, Deferred, all,
  ...
  return declare([BaseWidget], {
    ...
    _resetSearchDijitStyle: function() {
      ...
      setTimeout(lang.hitch(this, function() {
        if (this.searchDijit && this.searchDijit.domNode) {
          ...
          let sourcesBox = html.getMarginBox(this.searchDijit.sourcesBtnNode);
          let submitBox = html.getMarginBox(this.searchDijit.submitNode);
          let style = null;
          if (box.w) {
            ...
            if (isFinite(sourcesBox.w) && isFinite(submitBox.w)) {
              ...
              let inputGroup = query('.searchInputGroup', this.searchDijit.domNode)[0];

              if (inputGroup) {
                html.setStyle(inputGroup, style);
                let groupBox = html.getMarginBox(inputGroup);
                let extents = html.getPadBorderExtents(this.searchDijit.inputNode);
    ...
    _convertConfig: function(config) {
      let sourceDefs = array.map(config.sources, lang.hitch(this, function(source) {
        ...
        if (source && source.url && source.type === 'locator') {
          ...
          let searchLayer = new FeatureLayer(source.url || null, {
            outFields: ['*']
          });

          this.own(on(searchLayer, 'load', lang.hitch(this, function(result) {
            let flayer = result.layer;
            let template = this._getInfoTemplate(flayer, source, source.displayField);
            let fNames = null;
            ...
            if (convertedSource._featureLayerId) {
              let layerInfo = this.layerInfosObj
    ...
    _getInfoTemplate: function(fLayer, source, displayField) {
      let layerInfo = this.layerInfosObj.getLayerInfoById(source.layerId);
      let template = layerInfo && layerInfo.getInfoTemplate();
      let validTemplate = layerInfo && template;
    ...
    _getSourcePopupInfo: function(source) {
      if (source._featureLayerId) {
        let layerInfo = this.layerInfosObj.getLayerInfoById(source._featureLayerId);
        if (layerInfo) {
    ...
    _captureSelect: function(e) {
      let sourceIndex = this.searchDijit.activeSourceIndex;
      if (sourceIndex === 'all') {
        sourceIndex = this._getSourceIndexOfResult(e);
      }
      if (isFinite(sourceIndex) && esriLang.isDefined(sourceIndex)) {
        let source = this.searchDijit.sources[sourceIndex];
        if (source && 'featureLayer' in source) {
          let popupInfo = this._getSourcePopupInfo(source);
          let notFormatted = (popupInfo && popupInfo.showAttachments) ||
          ...
          if (!notFormatted) {
            let formatedAttrs = this._getFormatedAttrs(
    ...
    _getSourceIndexOfResult: function(e) {
      if (this.searchResults) {
        for (let i in this.searchResults) {
          let sourceResults = this.searchResults[i];
          let pos = array.indexOf(sourceResults, e);
          if (pos > -1) {
    ...
    _formatContent: function(title, fLayer, displayField, graphic) {
      ...
      if (graphic && graphic.attributes && fLayer && fLayer.url) {
        ...
        let displayValue = graphic.attributes[displayField];
    ...
    _getFormatedAttrs: function(attrs, fields, typeIdField, types, popupInfo) {
      function getFormatInfo(fieldName) {
        if (popupInfo && esriLang.isDefined(popupInfo.fieldInfos)) {
          for (let i = 0, len = popupInfo.fieldInfos.length; i < len; i++) {
            let f = popupInfo.fieldInfos[i];
      ...
      array.forEach(fields, lang.hitch(this, function(_field, i) {
        ...
        let isCodeValue = !!(_field.domain && _field.domain.type === 'codedValue');
        let isDate = _field.type === 'esriFieldTypeDate';
        let isTypeIdField = typeIdField && (_field.name === typeIdField);
        let fieldAlias = _field.name;
    ...
    getCodeValueFromTypes: function(field, typeIdField, types, obj, aliasAttrs) {
      ...
      if (typeIdField && types && types.length > 0) {
        let typeChecks = array.filter(types, lang.hitch(this, function(item) {
          // value of typeIdFild has been changed above
          return item.name === obj[typeIdField];
        }));
        let typeCheck = (typeChecks && typeChecks[0]) || null;
      ...
      let fieldAlias = field.name;
      let _value = codeValue !== null ? codeValue : aliasAttrs[fieldAlias];
      return _value || isFinite(_value) ? _value : '';
    },

    _resetSelectorPosition: function(cls) {
      let layoutBox = html.getMarginBox(window.jimuConfig.layoutId);
      query(cls, this.domNode).forEach(lang.hitch(this, function(menu) {
        let menuPosition = html.position(menu);
        if (html.getStyle(menu, 'display') === 'none') {
          return;
        }
        let dijitPosition = html.position(this.searchDijit.domNode);
        let up = dijitPosition.y - 2;
        let down = layoutBox.h - dijitPosition.y - dijitPosition.h;
    ...
    _onSearchResults: function(evt) {
      let sources = this.searchDijit.get('sources');
      let activeSourceIndex = this.searchDijit.get('activeSourceIndex');
      let value = this.searchDijit.get('value');
      let htmlContent = '';
      ...
      if (results && evt.numResults > 0) {
        ...
        for (let i in results) {
          if (results[i] && results[i].length) {
            let name = sources[parseInt(i, 10)].name;
            if (sources.length > 1 && activeSourceIndex === 'all') {
              htmlContent += '<div title="' + name + '" class="menuHeader">' + name + '</div>';
            }
            htmlContent += '<ul>';
            let partialMatch = value;
            let r = new RegExp('(' + partialMatch + ')', 'gi');
            let maxResults = sources[i].maxResults;

            for (let j = 0, len = results[i].length; j < len && j < maxResults; j++) {
              let text = esriLang.isDefined(results[i][j].name)
    ...
    _onSelectSearchResult: function(evt) {
      ...
      let result = null;
      let dataSourceIndex = html.getAttr(target, 'data-source-index');
      let dataIndex = parseInt(html.getAttr(target, 'data-index'), 10);
    ...
    _onSelectResult: function(e) {
      ...
      let dataSourceIndex = e.sourceIndex;
      let sourceResults = this.searchResults[dataSourceIndex];
      let dataIndex = 0;
      let that = this;

      let getGraphics = function(layer, fid) {
        let graphics = layer.graphics;
        let gs = array.filter(graphics, function(g) {
          return g.attributes[layer.objectIdField] === fid;
        });
        return gs;
      };
      let showPopupByFeatures = function(features) {
        let location = null;
      ...
      for (let i = 0, len = sourceResults.length; i < len; i++) {
      ...
      query('li', this.searchResultsNode)
        .forEach(lang.hitch(this, function(li) {
          ...
          let title = html.getAttr(li, 'title');
          let dIdx = html.getAttr(li, 'data-index');
          let dsIndex = html.getAttr(li, 'data-source-index');
      ...
      let layer = this.map.getLayer(e.source._featureLayerId);

      if (layer && this.config.showInfoWindowOnSelect) {
        let gs = getGraphics(layer, e.result.feature.__attributes[layer.objectIdField]);
        if (gs.length > 0) {
    ...
    _showResultMenu: function() {
      ...
      let groupNode = query('.searchInputGroup', this.searchDijit.domNode)[0];
      if (groupNode) {
        let groupBox = html.getMarginBox(groupNode);
        let style = {
          width: groupBox.w + 'px'
        };
        if (window.isRTL) {
          let box = html.getMarginBox(this.searchDijit.domNode);

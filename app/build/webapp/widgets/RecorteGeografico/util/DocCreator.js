/* global define zingchart moment pdfMake */
/* jslint nomen:true */
/// <reference path="../jsapi_vsdoc12_v33.js" />
define([
  'require',

  'dojo/io/script'
], function(require) {
  'use strict';
  let DocCreator = {};

  let create = function(data) {
    let button = document.getElementById('dropdownMenu3');

    let canvas1;
    let canvas2;
    let canvas3;
    let canvas4;
    let treeMap;
    let treeMap2;
    let canvasImg1;
    let canvasImg2;
    let canvasImg3;
    let canvasImg4;

    if (button.innerText === 'Unidade ') {
      canvas1 = document.getElementById('chartsToPDF_canvas_1');
      canvas2 = document.getElementById('chartsToPDF_canvas_2');
      canvas3 = document.getElementById('carousel_inner_canvas_3');
      canvasImg1 = canvas1.toDataURL('image/png', 1.0);
      canvasImg2 = canvas2.toDataURL('image/png', 1.0);
      canvasImg3 = canvas3.toDataURL('image/png', 1.0);
    } else {
      canvas1 = document.getElementById('chartsToPDF_canvas_1');
      canvas2 = document.getElementById('chartsToPDF_canvas_2');
      canvas3 = document.getElementById('chartsToPDF_canvas_3');
      canvas4 = document.getElementById('chartsToPDF_canvas_4');
      treeMap = zingchart.exec('carousel_inner_chartdiv_3', 'getimagedata', {
        format: 'png'
      });
      treeMap2 = zingchart.exec('carousel_inner_chartdiv_4', 'getimagedata', {
        format: 'png'
      });
      canvasImg1 = canvas1.toDataURL('image/png', 1.0);
      canvasImg2 = canvas2.toDataURL('image/png', 1.0);
      canvasImg3 = canvas3.toDataURL('image/png', 1.0);
      canvasImg4 = canvas4.toDataURL('image/png', 1.0);
    }

    let titleTablePesquisa = 'Tabela - Pesquisas';
    let titleTableTransmissao = 'Tabela - Transmissões';
    let objectsNotUnits = [
      {
        columns: [{
          image: canvasImg1,
          width: 235
        },
        {
          image: canvasImg2,
          width: 235
        }
        ],
        columnGap: 35,
        margin: [0, 0, 0, 10]
      },
      {
        columns: [{
          image: canvasImg3,
          width: 235
        },
        {
          image: canvasImg4,
          width: 235
        }
        ],
        columnGap: 35
      },
      {
        columns: [{
          text: 'Transmissão: Tipologia (Treemap)',
          pageBreak: 'before'
        },
        {
          text: 'Pesquisa: Tipologia (Treemap)',
          pageBreak: 'before'
        }
        ],
        columnGap: 20,
        margin: [0, 0, 0, 10]
      },
      {
        columns: [{
          image: treeMap,
          width: 235
        },
        {
          image: treeMap2,
          width: 235
        }
        ],
        columnGap: 35
      }];
    let objectsOfUnits = [
      {
        columns: [{
          image: canvasImg1,
          width: 235
        },
        {
          image: canvasImg2,
          width: 235
        }
        ],
        columnGap: 35,
        margin: [0, 0, 0, 10]
      },
      {
        columns: [{
          image: canvasImg3,
          width: 235
        }]
      }];

    let graphObjects;

    if (button.innerText === 'Unidade ') {
      graphObjects = objectsOfUnits;
    } else {
      graphObjects = objectsNotUnits;
    }

    if (!data.tableTransmissaoToPDFRows || data.tableTransmissaoToPDFRows.length === 0) {
      data.tableTransmissaoToPDFTitles = [];
      data.tableTransmissaoToPDFRows = [[]];
      titleTableTransmissao = '';
    }
    if (!data.tablePesquisaToPDFRows || data.tablePesquisaToPDFRows.length === 0) {
      data.tablePesquisaToPDFTitles = [];
      data.tablePesquisaToPDFRows = [[]];
      titleTablePesquisa = '';
    }
    let docDefinition = {
      pageMargins: [ 40, 80, 40, 60 ],
      header: {
        image: "data:image/png;base64,",
        width: 100,
        alignment: 'center',
        margin: [0, 10]
      },
      footer: function(currentPage, pageCount) {
        return {
          text: 'Página | ' + currentPage.toString(),
          alignment: 'right',
          margin: [0, 0, 20, 0],
          fontSize: 10
        };
      },
      content: [{
        text: 'Período pesquisado: De ' + moment(data.dataDeRecorte.value, 'DD-MM-YYYY').format('DD/MM/YYYY') + ' até ' + moment(data.dataAteRecorte.value, 'DD-MM-YYYY').format('DD/MM/YYYY'),
        margin: [0, 0, 0, 0],
        alignment: 'center'
      },
      {
        columns: [
          // Sumario
          {
            width: 250,
            text: [{
              text: 'Transmissões\n',
              bold: true
            },
            {
              text: 'Territoriais: ',
              fontSize: 10
            },
            {
              text: data.qtdTransmissaoTerritorialLabel.innerText + '  ',
              bold: true,
              fontSize: 10
            },
            {
              text: data.valorTransmissaoTerritorialLabel.innerText + '\n',
              fontSize: 10
            },
            {
              text: 'Prediais:  ',
              fontSize: 10
            },
            {
              text: data.qtdTransmissaoPredialLabel.innerText + '  ',
              bold: true,
              fontSize: 10
            },
            {
              text: data.valorTransmissaoPredialLabel.innerText + '\n',
              fontSize: 10
            },
            {
              text: 'Total:  ',
              fontSize: 10
            },
            {
              text: data.qtdTransmissaoTotalLabel.innerText + '  ',
              bold: true,
              fontSize: 10
            },
            {
              text: data.valorTransmissaoTotalLabel.innerText + '\n\n',
              fontSize: 10
            },
            {
              text: 'Pesquisas\n',
              bold: true
            },
            {
              text: 'Territoriais:  ',
              fontSize: 10
            },
            {
              text: data.qtdPesquisaTerritorialLabel.innerText + '  ',
              bold: true,
              fontSize: 10
            },
            {
              text: data.valorPesquisaTerritorialLabel.innerText + '\n',
              fontSize: 10
            },
            {
              text: 'Prediais:  ',
              fontSize: 10
            },
            {
              text: data.qtdPesquisaPredialLabel.innerText + '  ',
              bold: true,
              fontSize: 10
            },
            {
              text: data.valorPesquisaPredialLabel.innerText + '\n',
              fontSize: 10
            },
            {
              text: 'Total:  ',
              fontSize: 10
            },
            {
              text: data.qtdPesquisaTotalLabel.innerText + '  ',
              bold: true,
              fontSize: 10
            },
            {
              text: data.valorPesquisaTotalLabel.innerText,
              fontSize: 10
            }
            ]
          },
          // Fim do Sumario
          // Mapa
          {
            image: data.urlOverview,
            width: 250
          }
          // Fim do mapa
        ],
        margin: [0, 15, 0, 0]
      }, // termino da primeira coluna
      {
        text: 'Gráficos da área pesquisada:',
        margin: [0, 15, 0, 0]
      },
      graphObjects,
      {
        text: titleTableTransmissao,
        margin: [0, 20, 0, 5],
        bold: true
      },
      {
        style: ['headerTable', 'textsmall'],
        table: {
          widths: [63.61, 63.61, 63.61, 63.61, 63.61, 63.61, 63.61],
          body: [data.tableTransmissaoToPDFTitles]
        },
        layout: {
          hLineColor: function(i, node) {
            if (!node.table.body) {
              return 'gray';
            }
            return (i === 0 || i === node.table.body.length) ? 'gray' : 'gray';
          },
          vLineColor: function(i, node) {
            if (!node.table.widths) {
              return 'gray';
            }
            return (i === 0 || i === node.table.widths.length) ? 'gray' : 'gray';
          }
          // paddingLeft: function(i, node) { return 4; },
          // paddingRight: function(i, node) { return 4; },
          // paddingTop: function(i, node) { return 2; },
          // paddingBottom: function(i, node) { return 2; },
          // fillColor: function (i, node) { return null; }
        }
      },
      {
        style: 'smallTable',
        table: {
          widths: [63.61, 63.61, 63.61, 63.61, 63.61, 63.61, 63.61],
          body:
                            data.tableTransmissaoToPDFRows
        },
        layout: {
          hLineColor: function(i, node) {
            if (!node.table.body) {
              return 'gray';
            }
            return (i === 0 || i === node.table.body.length) ? 'gray' : 'gray';
          },
          vLineColor: function(i, node) {
            if (!node.table.widths) {
              return 'gray';
            }
            return (i === 0 || i === node.table.widths.length) ? 'gray' : 'gray';
          }
          // paddingLeft: function(i, node) { return 4; },
          // paddingRight: function(i, node) { return 4; },
          // paddingTop: function(i, node) { return 2; },
          // paddingBottom: function(i, node) { return 2; },
          // fillColor: function (i, node) { return null; }
        }
      },
      {
        text: titleTablePesquisa,
        margin: [0, 10, 0, 5],
        bold: true
      },
      {
        style: ['headerTable', 'textsmall'],
        table: {
          widths: [55.65, 55.65, 55.65, 55.65, 55.65, 55.65, 55.65, 55.65],
          body: [data.tablePesquisaToPDFTitles]
        },
        layout: {
          hLineColor: function(i, node) {
            if (!node.table.body) {
              return 'gray';
            }
            return (i === 0 || i === node.table.body.length) ? 'gray' : 'gray';
          },
          vLineColor: function(i, node) {
            if (!node.table.widths) {
              return 'gray';
            }
            return (i === 0 || i === node.table.widths.length) ? 'gray' : 'gray';
          }
          // paddingLeft: function(i, node) { return 4; },
          // paddingRight: function(i, node) { return 4; },
          // paddingTop: function(i, node) { return 2; },
          // paddingBottom: function(i, node) { return 2; },
          // fillColor: function (i, node) { return null; }
        }
      },
      {
        style: 'smallTable',
        table: {
          widths: [55.65, 55.65, 55.65, 55.65, 55.65, 55.65, 55.65, 55.65],
          body:
                            data.tablePesquisaToPDFRows
        },
        layout: {
          hLineColor: function(i, node) {
            if (!node.table.body) {
              return 'gray';
            }
            return (i === 0 || i === node.table.body.length) ? 'gray' : 'gray';
          },
          vLineColor: function(i, node) {
            if (!node.table.widths) {
              return 'gray';
            }
            return (i === 0 || i === node.table.widths.length) ? 'gray' : 'gray';
          }
          // paddingLeft: function(i, node) { return 4; },
          // paddingRight: function(i, node) { return 4; },
          // paddingTop: function(i, node) { return 2; },
          // paddingBottom: function(i, node) { return 2; },
          // fillColor: function (i, node) { return null; }
        }
      }

      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true
        },
        boldtext: {
          bold: true
        },
        textsmall: {
          fontSize: 10
        },
        smallTable: {
          fontSize: 8
        },
        headerTable: {
          bold: true
        }
      }
    };

    return pdfMake.createPdf(docDefinition).open();
  };

  DocCreator.create = create;

  return DocCreator;
});

  ...
  function(
    ...
    return declare([BaseWidget], {
      ...
      startup: function() {
        ...
        $(document).ready(function() {
          ...
          $('#geocode').focusin(function () {
              ...
              console.info("entrou no add event listener", geocodes);
      ...
      searchImovel: function() {
        ...
        selfCalculo.getDataFromStm(function(err, stmData) {
          if (err || !stmData) {
            ...
            if (!stmData) {
              DocumentUtil.showSpinner(false);
              DocumentUtil.showMessage('warning', 'Esta inscrição não possui dados no STM!', true, null, 'comunicacaoStmNull');
            }
          } else {
            ...
            selfCalculo.searchTableLote(function(tableLoteFeature) {
              ...
              if (valores && valores.isPredial) {
                ...
                if (stmData.imovel_codigo_logradouro.length >= 6) {
              	...
              	let testada = stmData.bci_componentes["0"].testada.tamanho;
      ...
      searchLote: function(wkid, callback) {
        ...
        query.outFields = [layerFieldName, shapeFieldName, 'sl_cadastro.SDE.TB_LOTES.DE_COD_SQCODLOG', 'sl_cadastro.SDE.TB_LOTES.NU_AREA_TERRENO'];
        // query.outFields = ['*'];
      ...
      searchPlantaDeValores: function(testada, sqcodlog, areaTerreno, valores) {
        ...
        if (sqcodlog && areaTerreno) {
          ...
          DocumentUtil.showMessage('warning', 'Não foi possível identificar a relação do imóvel com uma face de quadra (planta de valores)!', true, null, 'realizarCalculoError');
        }
        function showResults(result) {
      ...
      getValoresParaCalculo: function(data, callback) {
        ...
        if (data) {
          ...
          if (isPredial && (tipologia && padrao && areaConstruida && idade && conservacao)) {
            ...
            DocumentUtil.showMessage('warning', 'Não foi possível realizar o calculo! Valores não preenchidos no STM!', true, null, 'realizarCalculoError');
            return null;
          } else {
            return {
              isPredial: isPredial,
      ...
      getGeocodeBuffer: function(featureSelected, unidadeSelected) {
        ...
        function showBuffer(bufferedGeometries) {
          ...
          array.forEach(bufferedGeometries, function(circleGeometry) {
            ...
            query.outFields = ['sl_cadastro.SDE.Lote.GEOC_RENU', 'sl_cadastro.SDE.TB_LOTES.DE_GEOCODE_LOTE', 'sl_cadastro.SDE.Lote.OBJECTID_1'];

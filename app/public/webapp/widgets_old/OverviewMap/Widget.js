  ...
  let clazz = declare([BaseWidget], {
    ...
    _updateDomPosition: function(attachTo) {
      if (this.overviewMapDijit) {
        ...
        let _position = this._getOverviewPositionByAttach(attachTo);
        lang.mixin(initPos, _position);
        let style = utils.getPositionStyle(initPos);
    ...
    createOverviewMap: function(visible) {
      let json = lang.clone(this.config.overviewMap);
      json.map = this.map;
      ...
      let _isShow = json.visible;
      json.visible = false;

      let _hasMaximizeButton = 'maximizeButton' in json;

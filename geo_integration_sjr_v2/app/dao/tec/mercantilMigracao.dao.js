        ...
        function(callback) {
            boundConfigModel
                ...
                .then(async function(row) {
                    var qtdMercantil = await mercantilDAO.count()
                    if (qtdMercantil > 0) {
...
function migrate(next) {
    ...
    async.whilst(
        ...
        async function(done) {
            ...
            var list;
            if (count === 0) {
                list = await mercantilDAO.fillList('NU_COD_BAIRRO IS NULL AND DE_SITUACAO=\'ATIVO\'', 'TecGeo.IMOBILIARIO', 'NU_SEQUENCIAL');
            } else {
                list = await mercantilDAO.fillList('NU_COD_BAIRRO = \'' + codBairro + '\' AND DE_SITUACAO=\'ATIVO\'', 'TecGeo.IMOBILIARIO', 'NU_SEQUENCIAL');
            }

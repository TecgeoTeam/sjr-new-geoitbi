...
public class ConsultaPavimentos implements IConsultaStrategy {
	private IDestinoUnidadesPropRepository unidadeRepository;
	private DAOUtilUnidades daoUtils;
	
	public ConsultaPavimentos(IDestinoUnidadesPropRepository unidadeRepository) {
		super();
	...
	public List<UnidadeDestino> consultarDados(FiltroCamposUnidade filtro, String inscricao) {
		...
		if (filtro.getPavimentos() != null) {
			if (filtro.getOperador().equals("="))
				resultado.addAll(unidadeRepository.findByGeocodeLoteAndPavimentosLike(inscricao, filtro.getPavimentos()));
			if (filtro.getOperador().equals("<>"))
				resultado.addAll(unidadeRepository.findByGeocodeLoteAndPavimentosNotLike(inscricao, filtro.getPavimentos()));
		}

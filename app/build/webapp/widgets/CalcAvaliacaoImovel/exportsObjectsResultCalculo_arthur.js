let image = "data:image/png;base64,"; 



function paramsCalc(codigoSetor, coodY, coodX,areaDoTerrenoTotal, pavimentacao, testadaDoTerreno, rendaSetor, valorDoMetroQuadrado, padraoConserv, tipologia, idadeDoImovel, drenagem, esgoto, areaConst, zonaUrbana, topografia, padraoConstrucao){
    if(['07','08','10','11'].includes(codigoSetor)){
        return [
            {
                text: [{ text: 'VU', bold: true }, ' = ', 'Valor Unitário;'],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_N', bold: true }, ' = ', 'Coordenada N do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_E', bold: true }, ' = ', 'Coordenada E do centroide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Area_Ter', bold: true }, ' = ', 'Área do Terreno ou Fração Ideal (em casos de Condomínios) => ', areaDoTerrenoTotal],
                fontSize: 10
            },
            {
                text: [{ text: 'Pavimentacao', bold: true }, ' = ', ' = Possui pavimentação => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Frente_Ter', bold: true }, ' = ', 'Testada principal(m) ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Renda_Setor', bold: true }, ' = ', 'Renda do Setor => ', "5.351,68"],
                fontSize: 10
            }
        ]
    }else if(['01'].includes(codigoSetor)){
        return [
            {
                text: [{ text: 'VU ', bold: true }, '= Valor Unitário => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_N', bold: true }, ' = Coordenada N do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_E', bold: true }, ' = Coordenada E do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Area_Ter', bold: true }, ' = Área do Terreno ou Fração Ideal (em casos de Condomínios) =>', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Padrao_Converva', bold: true }, ' = Padrão da Conservação => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Tipo', bold: true }, ' = Tipologia do Imóvel => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Frente_Ter', bold: true }, ' = Testada principal(m) => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Idade', bold: true }, ' – Idade do Ímovel => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Renda_Setor', bold: true }, ' = Renda do Setor (V003) da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            }
        ]
    }else if(['04','05','06'].includes(codigoSetor)){
        return [
            {
                text: [{ text: 'VU ', bold: true }, '= Valor Unitário => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_N', bold: true }, ' = Coordenada N do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_E', bold: true }, ' = Coordenada E do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Area_Ter', bold: true }, ' = Área do Terreno ou Fração Ideal (em casos de Condomínios) =>', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Rede_Drenagem', bold: true }, ' = Existência de rede de drenagem => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Frente_Ter', bold: true }, ' – Testada principal(m) => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Renda_Setor', bold: true }, ' = Renda do Setor (V003) da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            }
        ]
    }else if(['12','14','15','16'].includes(codigoSetor)){
        return [
            {
                text: [{ text: 'VU ', bold: true }, '= Valor Unitário => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_N', bold: true }, ' = Coordenada N do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_E', bold: true }, ' = Coordenada E do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Area_Ter', bold: true }, ' = Área do Terreno ou Fração Ideal (em casos de Condomínios) =>', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Pavimentacao', bold: true }, ' = Possui pavimentação => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Frente_Ter', bold: true }, ' – Testada principal(m) => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Renda_Setor', bold: true }, ' = Renda do Setor => ', "5.351,68"],
                fontSize: 10
            }
        ]
    }else if(['02','03', '13'].includes(codigoSetor)){
        return [
            {
                text: [{ text: 'VU ', bold: true }, '= Valor Unitário => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_N', bold: true }, ' = Coordenada N do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_E', bold: true }, ' = Coordenada E do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Area_Ter', bold: true }, ' = Área do Terreno ou Fração Ideal (em casos de Condomínios) =>', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Rede_Drenagem', bold: true },' = Existência de rede de drenagem => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Area_const', bold: true },' = Área construída => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Conservaco', bold: true }, ' = Conservação do imóvel => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Idade', bold:true},' = Idade do imóvel => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Frente_Ter', bold: true }, ' – Testada principal(m) => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Renda_Setor', bold: true }, ' = Renda do Setor (V003) da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            }
        ]
    }else if(['09'].includes(codigoSetor)){
        return [
            {
                text: [{ text: 'VU ', bold: true }, '= Valor Unitário => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_N', bold: true }, ' = Coordenada N do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_E', bold: true }, ' = Coordenada E do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Area_Ter', bold: true }, ' = Área do Terreno ou Fração Ideal (em casos de Condomínios) =>', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Rede_Esgoto', bold: true},' = Existência de rede de esgoto => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Frente_Ter', bold: true},' = Testada principal(m) => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Area_Const', bold: true},' = Área construída => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Padrao_Conserva', bold: true}, ' = Estado de Conservação => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Idade', bold: true },' = Idade do imóvel => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Renda_Setor', bold: true }, ' = Renda do Setor (V003) da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            }
        ]
    }else if(['16'].includes(codigoSetor)){
        return [
            {
                text: [{ text: 'VU ', bold: true }, '= Valor Unitário => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_N', bold: true }, ' = Coordenada N do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Coord_E', bold: true }, ' = Coordenada E do centróide da Planta de Valor => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Area_Ter', bold: true }, ' = Área do Terreno ou Fração Ideal (em casos de Condomínios) =>', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Topografia', bold: true},' = Topografia do Terreno => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Frente_Ter', bold: true},' = Testada principal(m) => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Area_Const', bold:true},' = Área construída => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Padrao_Conserva', bold:true},' = Estado de Conservação => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Idade', bold:true},' = Idade do imóvel => ', "5.351,68"],
                fontSize: 10
            },
            {
                text: [{ text: 'Zona_Urbana', bold: true },' = Se o imóvel está localizado em zona urbana => ', "5.351,68"],
                fontSize: 10
            }
        ]
    }
}

  function calcDetalhado(codigoSetor){
    if(['07','08','10','11'].includes(codigoSetor)){
        return {
            text: [{text: 'Vut: \n', bold: true},
              'VuT = \n ( -1244,7487 + 1,9918227e-11 * COORD_N² + -1,8324684e-09 *' +
                    '\n COORD_E² + 1,801289e-09 * AREA_TER ² + 2,4943315 *' +
                    '\n PAVIMENTO + 0,0019993246 * FRENTE_TER² + 3,0366837e-06 * RENDA_SETOR) ²'],
            style: 'contas'
        }
    }else if(['01'].includes(codigoSetor)){
        return {
            text: [ {text:'VU: \n', bold: true} ,
            'VuT = 3,2735899e+159 * e^(-3,1536032e-12 * COORD_N²)'+
            '\n * e^(-1,7332165e-10 * COORD_E²) * e^(-14358,838 * 1/RENDA_SETOR)'+
             '\n * e^(-1,9996231e-07 * AREA_TER²) * e^(0,00036361237 * AREA_CONST) *' +
             '\n e^(-0,0089674519 * PADRAO_CONSERVA²) * e^(0,62922743 * TIPO½ ) * e^(0,0055415258 * FRENTE_TER²) * e^(-4,9683898e-05 * IDADE²)'],
            style: 'contas'
        }
    }else if(['04','05','06'].includes(codigoSetor)){
        return {
            text: [{text:'VU: \n', bold: true},
            'Ln(VuT) = 1 / (0,9701669 - 1,9110612e-14 * COORD_N² + 2,6379857e-12 *'+
                    '\n COORD_E² - 4,7631089*1 / AREA_TER² - 0,018723065 * REDE_DRENAGEM'+
                    '\n - 2,6963265e-06  *FRENTE_TER² + 3,0296439e+08 * 1 / RENDA_SETOR²)²'],
            style: 'contas'
        }
    }else if(['12','14','15','16'].includes(codigoSetor)){
        return {
            text: [{text: 'VU: \n', bold: true},
            {text: 'Fator Y', bold: true}, '= -423628850000000000 * (Coord_N * Coord_N) \n' ,
            {text: 'Fator X', bold: true}, '= 0.00000000041166656 * (Coord_E * Coord_E) \n' ,
              {text: 'Fator Area Terreno', bold: true}, '= -0.030671871 * RAIZ(Area_Ter) \n' ,
              {text: 'Fator Pavimentacao', bold: true},  '= 1.3467551 * Pavimentacao \n' ,
               {text: 'Fator Testada Terreno', bold: true},  '= 0.00067587177 * (FRENTE_TER * FRENTE_TER) \n' ,
               {text: 'Fator Renda', bold: true}, '= 0.0000000000041779087 * (Renda_Setor * Renda_Setor) \n' ,
                {text: 'Valor Estatistico', bold: true},  '= 4356.8578 + Fator Y + Fator X + Fator Area Terreno + Fator Pavimentacao + Fator Testada Terreno + Fator Renda \n' ,
                {text: 'Valor Metro Quadrado', bold: true}, '= Valor Estatistico * Valor Estatistico'],
            style: 'contas'
        }
    }else if(['02','03', '13'].includes(codigoSetor)){
        return {
            text: [{text:'VU: \n', bold: true},
            {text: 'Fator Y', bold: true}, '= -1.3209366e+16 * (1/(Coord_N * Coord_N)) \n' ,
            {text: 'Fator X', bold: true},  '= 1.5919961e-11 * (Coord_E * Coord_E) \n' ,
            {text:'Fator Rede Drenagem', bold: true},  '= 0.193399 * Rede_Drenagem \n' ,
            {text: 'Fator Renda', bold: true},  '= -169284.71 * (1 / Renda_Setor) \n' ,
            {text: 'Fator Area Terreno', bold: true},  '= 0.085658933 * RAIZ(Area_Ter) \n' ,
            {text: 'Valor Estatistico', bold: true},  '= 144.32798 + Fator Y + Fator X + Fator Rede Drenagem + Fator Renda + Fator Area Terreno \n' ,
                {text: 'Valor Total do Imovel', bold: true},  '= 2.718281828459045 ^ Valor Estatistico \n' ,
                {text: 'Valor Metro Quadrado', bold: true},  '=Valor Total do Imovel  / Area_Ter'],
            style: 'contas'
        }
    }else if(['09'].includes(codigoSetor)){
        return {
            text: [{text: 'VU: \n', bold: true},
             {text:'Fator Y', bold: true}, '= -16758505000000000 * 1 / (Coord_N * Coord_N) \n',
            {text: 'Fator X', bold: true},  '= -0.000000000064551228 * (Coord_E * Coord_E) \n' ,
            {text: 'Fator Rede Drenagem', bold: true},  '= 0.79259538 * REDE_ESGOTO \n' ,
            {text: 'Fator Testada Terreno', bold: true}, '= -0.014018364 * FRENTE_TER \n' ,
            {text: 'Fator Renda', bold: true},  '= 0.075673609 *  LOG(Renda_Setor) \n' ,
            {text: 'Fator Area Terreno', bold: true},  '= 0.35229635 * LOG(Area_Ter) \n' ,
            {text: 'Valor Estatistico', bold: true}, '= 206.25131 + Fator Y + Fator X + Fator Rede Drenagem + Fator Testada Terreno + Fator Renda + Fator Area Terreno \n' ,
                {text: 'Valor Total do Imovel', bold: true},  '= 2.718281828459045 ^ Valor Estatistico \n' ,
                {text:'Valor Metro Quadrado', bold: true},  '=Valor Total do Imovel  / Area_Ter'],
            style: 'contas'
        }
    }else if(['16'].includes(codigoSetor)){
        return {
            text: [{text:'VU: \n', bold: true},
            {text:'Fator Y', bold: true}, '= -423628850000000000 / (Coord_N * Coord_N) \n' ,
            {text: 'Fator X', bold: true}, '= 0.00000000041166656 * (Coord_E * Coord_E) \n' ,
            {text: 'Fator Area Terreno', bold: true}, '= -0.030671871 * RAIZ(Area_Ter) \n' ,
            {text: 'Fator Pavimentacao', bold: true}, '= 1.3467551 * Pavimentacao \n' ,
            {text: 'Fator Testada Terreno', bold: true},  '= 0.00067587177 * (FRENTE_TER * FRENTE_TER) \n' ,
            {text: 'Fator Renda', bold: true},  '= 0.0000000000041779087 * (Renda_Setor * Renda_Setor) \n' ,
                {text: 'Valor Estatistico', bold: true}, '= 4356.8578 + Fator Y + Fator X + Fator Area Terreno + Fator Pavimentacao + Fator Testada Terreno + Fator Renda \n' ,
                {text: 'Valor Metro Quadrado', bold: true}, '= Valor Estatistico * Valor Estatistico'],
            style: 'contas'
        }
    }
}

function resolucaoCalc(codigoSetor, coodY, coodX, areaDoTerrenoTotal, pavimentacao, testadaDoTerreno, rendaSetor, valorDoMetroQuadrado, TipoDoImovel, drenagem, fatorY, fatorX, fatorAreaTerreno,
    fatorpavimentacao,
    fatorTestadaTerreno,
    fatorRenda,
    valorEstatistico1,
    fatorRedeDrenagem,
    valorTotalDoImovel,
    valorEstatistico2 ){
    if(['07','08','10','11'].includes(codigoSetor)){
        return {
            text: [{text: 'VU: \n', bold: true},
            'VuT =\n  ( -1244,7487 + 1,9918227e-11 * ' + "5.351,68" + '² + -1,8324684e-09 * ' + "5.351,68"
                 + '\n ² + 1,801289e-09 * ' + "5.351,68" + ' ² + 2,4943315 * ' 
                 + "5.351,68" + ' + 0,0019993246 * ' +
                 "5.351,68" + '² + 3,0366837e-06 * ' + "5.351,68" + ' )² = ' + "5.351,68"],
            margin: [0, 0, 0, 5],
            style: 'contas'
        }
    }else if(['01'].includes(codigoSetor)){
        return  {
            text: [ {text:'VU: \n', bold: true},
             'VuT = 3,2735899e+159 * e^(-3,1536032e-12 * ' + "5.351,68" + '²) * e^(-1,7332165e-10 * ' + "5.351,68" + '²) * e^(-14358,838 * 1/' + "5.351,68" + ') * e^(-1,9996231e-07 * ' + "5.351,68" + '²) * e^(0,00036361237 * ' + "5.351,68" + ') * e^(-0,0089674519 * ' + "5.351,68" + '²) * e^(0,62922743 * ' + "5.351,68" + '½ ) * e^(0,0055415258 * ' + "5.351,68" + '²) * e^(-4,9683898e-05 * ' + "5.351,68" + '²) = ' + "5.351,68"],
            margin: [0, 0, 0, 5],
            style: 'contas'
        }
    }else if(['04','05','06'].includes(codigoSetor)){
        return {
            text: [{text: 'VU: \n', bold: true},
            'VU = 1 / (0,9701669 - 1,9110612e-14 * ' + "5.351,68" + '² + 2,6379857e-12 * ' + "5.351,68" + '² - 4,7631089*1 / ' + "5.351,68" + '² - 0,018723065 * ' + "5.351,68" + ' - 2,6963265e-06  * ' + "5.351,68" + '² + 3,0296439e+08 * 1 / ' + "5.351,68" + '²)² = ' + "5.351,68"],
            margin: [0, 0, 0, 5],
            style: 'contas'
        }
    }else if(['12','14','15','16'].includes(codigoSetor)){
        return {
            text: [{text: 'VU: \n', bold: true}, 
            {text: 'Fator Y', bold: true}, '= -423628850000000000 * (' + "5.351,68" + ' * ' +  "5.351,68" +') = ' + "5.351,68" + '\n' ,
            {text: 'Fator X', bold: true},  '= 0.00000000041166656 * (' + "5.351,68" + ' * ' +  "5.351,68" +') = ' + "5.351,68" + '\n',
            {text: 'Fator Area Terreno', bold: true},  '= -0.030671871 * RAIZ('+ "5.351,68" +') = ' + "5.351,68" + '\n' ,
            {text: 'Fator Pavimentacao', bold: true}, '= 1.3467551 *' + "5.351,68" + ' = ' + "5.351,68" + '\n' ,
            {text: 'Fator Testada Terreno', bold: true}, '= 0.00067587177 * (' + "5.351,68" + ' * ' +  "5.351,68" +') = ' + "5.351,68" + '\n' ,
            {text: 'Fator Renda', bold: true},  '= 0.0000000000041779087 * (' + "5.351,68" + ' * ' +  "5.351,68" +') = ' + "5.351,68" +  '\n' ,
            {text: 'Valor Estatistico', bold: true}, '= 4356.8578 + ' + "5.351,68" + ' + ' + "5.351,68" + ' + ' + "5.351,68" + ' + ' + fatorpavimentacao + ' + ' + fatorTestadaTerreno + ' + ' +  "5.351,68"  + ' = ' + "5.351,68" + '\n' ,
            {text: 'Valor Metro Quadrado', bold: true},  '= ' +  "5.351,68" + ' * ' +  "5.351,68" +  ' = ' + "5.351,68" + '\n' ,
            {text: 'Valor Metro Quadrado', bold: true}, '= '+ 'R$' + "5.351,68"],
            margin: [0, 0, 0, 5],
            style: 'contas'
        }
    }else if(['02','03', '13'].includes(codigoSetor)){
        return {
            text: [{text: 'VU: \n', bold: true},
            {text: 'Fator Y', bold: true}, '= -1.3209366e+16 * (1/(' + "5.351,68" + ' * ' +  "5.351,68" +')) = ' + "5.351,68" + '\n' ,
            {text: 'Fator X', bold: true}, '= 1.5919961e-11 * (' + "5.351,68" + ' * ' +  "5.351,68" +') = ' + "5.351,68" + '\n'  ,
            {text: 'Fator Rede Drenagem', bold: true}, '= 0.193399 *' + "5.351,68" + ' = ' + "5.351,68" + '\n' ,
            {text: 'Fator Renda', bold: true}, '= -169284.71 * (1 /'  + "5.351,68" + ') = ' + "5.351,68" + '\n' ,
            {text: 'Fator Area Terreno', bold: true}, '= 0.085658933 * RAIZ(' + "5.351,68" +') = ' + "5.351,68" + '\n' ,
            {text: 'Valor Estatistico', bold: true}, '= 144.32798 + ' + "5.351,68" + ' + ' +  "5.351,68" + ' + ' + "5.351,68" + ' + ' + "5.351,68" + ' + ' + "5.351,68" + ' = ' + "5.351,68" + '\n' ,
                {text: 'Valor Total do Imovel', bold: true}, '= 2.718281828459045 ^ ' + "5.351,68" + ' = ' + "5.351,68" + '\n' ,
                {text: 'Valor Metro Quadrado', bold: true}, ' = ' + "5.351,68" + '/' + "5.351,68" +  ' = ' + "5.351,68" + '\n' ,
                {text: 'Valor Metro Quadrado', bold: true}, '= ' + 'R$' + "5.351,68"],
            margin: [0, 0, 0, 5],
            style: 'contas'
        }
    }else if(['09'].includes(codigoSetor)){
        return {
            text: [{text: 'VU: \n', bold: true}, 
            {text: 'Fator Y', bold: true}, '= -16758505000000000 * 1 / (' + "5.351,68" + ' * ' +  "5.351,68" +') = ' + "5.351,68" + '\n' ,
            {text: 'Fator X', bold: true}, '= -0.000000000064551228 * (' + "5.351,68" + ' * ' +  "5.351,68" +') = ' + "5.351,68" + '\n' ,
            {text: 'Fator Rede Drenagem', bold: true},  '= 0.79259538 *' + "5.351,68" + ' = ' + "5.351,68" + '\n' ,
            {text: 'Fator Testada Terreno', bold: true}, '= -0.014018364 *' + "5.351,68" + ' = ' + "5.351,68" + '\n' ,
            {text: 'Fator Renda', bold: true}, '= 0.075673609 *  LOG('+ "5.351,68"+') = ' + "5.351,68" + '\n' ,
            {text: 'Fator Area Terreno', bold: true},  '= 0.35229635 * LOG('+ "5.351,68"+') = ' + "5.351,68" + '\n' ,
                {text: 'Valor Estatistico', bold: true}, '= 206.25131 + ' + "5.351,68" + ' + ' + "5.351,68" + ' + ' + "5.351,68" + ' + ' + "5.351,68" + ' + ' + "5.351,68" + ' + ' + "5.351,68" + ' = ' + "5.351,68" + '\n' ,
                {text: 'Valor Total do Imovel', bold: true}, '= 2.718281828459045 ^ ' + "5.351,68" + ' = ' + "5.351,68" + '\n' ,
                {text: 'Valor Metro Quadrado', bold: true}, ' = ' + "5.351,68" +  '/' + "5.351,68" + ' = ' + "5.351,68" + '\n',
                {text: 'Valor Metro Quadrado', bold: true}, '= ' + 'R$' + "5.351,68"],
            margin: [0, 0, 0, 5],
            style: 'contas'
        }
    }else if(['16'].includes(codigoSetor)){
        return {
            text: [{text: 'VU: \n', bold: true}, 
            {text: 'Fator Y', bold: true}, '= -423628850000000000 / (' + "5.351,68" + ' * ' +  "5.351,68" +') = ' + "5.351,68" + '\n' ,
            {text: 'Fator X', bold: true}, '= 0.00000000041166656 * (' + "5.351,68" + ' * ' +  "5.351,68" +') = ' + "5.351,68" + '\n' ,
            {text: 'Fator Area Terreno', bold: true}, '= -0.030671871 * RAIZ('+ "5.351,68"+') = ' + "5.351,68" + '\n' ,
            {text: 'Fator Pavimentacao', bold: true}, '= 1.3467551 *' + "5.351,68" + ' = ' + "5.351,68" + '\n' ,
            {text: 'Fator Testada Terreno', bold: true}, '= 0.00067587177 * (' + "5.351,68" + ' * ' +  "5.351,68" +') = ' + "5.351,68" + '\n' ,
            {text: 'Fator Renda', bold: true}, '= 0.0000000000041779087 * (' + "5.351,68" + ' * ' +  "5.351,68" +') = ' + "5.351,68" + '\n' ,
                {text: 'Valor Estatistico', bold: true},  '= 4356.8578 + ' + "5.351,68" + ' + ' + "5.351,68" + ' + ' + "5.351,68" + ' + ' + fatorpavimentacao + ' + ' + fatorTestadaTerreno + ' + ' + "5.351,68" + ' = ' + "5.351,68" + '\n' ,
                {text: 'Valor Metro Quadrado', bold: true},  ' = ' + "5.351,68" + ' * ' + "5.351,68" + ' = ' + "5.351,68" + '\n' ,
                {text:'Valor Metro Quadrado', bold: true}, '= '+ 'R$' + "5.351,68"],
            margin: [0, 0, 0, 5],
            style: 'contas'
        }
    }
}



function pdfObjectsBlocos(
    coodY,
    coodX,
    areaDoTerrenoTotal,
    pavimentacao,
    testadaDoTerreno,
    rendaSetor,
    areaConst,
    padraoConserv,
    tipologia,
    idade,
    drenagem,
    esgoto,
    topografia,
    bloco,
    codigoSetor,
    zonaUrbana,
    TipoDoImovel,
    inscricao,
    valoresCalculados,
    valorDoMetroQuadrado,
    CUB,
    depreciacao,
    fatorY,
    fatorX,
    fatorAreaTerreno,
    fatorpavimentacao,
    fatorTestadaTerreno,
    fatorRenda,
    valorEstatistico1,
    fatorRedeDrenagem,
    valorTotalDoImovel,
    valorEstatistico2,
    padraoConstrucao,
    idadeDoImovel,
    estadoDeConservacao
) {
    var bloco = {
        content: [
            {
                image: image,
                width: 150,
                alignment: 'center',
                margin: [0,0,0,15]
            },
            {
                text: 'Cálculo para o valor da 3ª Avaliação',
                style: 'header'
            },
            {
                text: [{ text: 'Inscrição Imobiliária: ', bold: true}, "5.351,68"],
                fontSize: 10
            },
            {
                text: ['A inscrição imobiliária informada faz parte do ', { text: bloco, bold: true}, ' da planta de valores.'],
                fontSize: 10
            },
            {
                text: 'RESUMO DOS RESULTADOS',
                alignment: 'center',
                style: 'subheader',
            },
            {
                text: 'Valor do Terreno:',
                bold: true, 
                fontSize: 10
            },
            {
                text: [{ text: 'Mínimo ', bold: true}, '= R$',"455344"],
                fontSize: 10
            },
            {
                text: [{ text: 'Médio ', bold: true}, '= R$', "455344"],
                fontSize: 10
            },
            {
                text: [{ text: 'Máximo ', bold: true}, '= R$',"455344"],
                fontSize: 10
            },
            {
                text: [{ text: 'Valor da Edificação ', bold: true }, '= R$', "455344"],
                fontSize: 10,
                margin: [0, 10, 0, 10]
            },
            {
                text: 'Valor do Imóvel:',
                bold: true,
                fontSize: 10
            },
            {
                text: [{ text: 'Mínimo ', bold: true }, '= R$',"455344"],
                fontSize: 10
            },
            {
                text: [{ text: 'Médio ', bold: true }, '= R$', "455344"],
                fontSize: 10
            },
            {
                text: [{ text: 'Máximo ', bold: true }, '= R$',"455344"],
                fontSize: 10
            },
            {
                text: 'CÁLCULO DETALHADO',
                alignment: 'center',
                style: 'subheader'
            },
            {
                text: '1.	Cálculo do valor do Terreno',
                style: 'subheader2'
            },
            {
                text: '1.1.	Fórmula',
                style: 'subheader3'
            },
            calcDetalhado(codigoSetor),
            {
                text: '1.2.	Parâmetros utilizados no cálculo',
                style: 'subheader3'
            },
            paramsCalc(codigoSetor, coodY, coodX,areaDoTerrenoTotal, pavimentacao, testadaDoTerreno, rendaSetor, valorDoMetroQuadrado, padraoConserv, tipologia, idadeDoImovel, drenagem, esgoto, areaConst, zonaUrbana, topografia, padraoConstrucao),
            {
                text: 'OBS: Os demais valores são constantes da fórmula pertencentes a este bloco.',
                margin: [0, 5, 0, 0]
            },
            {
                text: '1.3.	Resolução do cálculo',
                style: 'subheader3'
            },
            resolucaoCalc(codigoSetor, coodY, coodX, areaDoTerrenoTotal, pavimentacao, testadaDoTerreno, rendaSetor, valorDoMetroQuadrado, TipoDoImovel, drenagem, fatorY, fatorX, fatorAreaTerreno,
                fatorpavimentacao,
                fatorTestadaTerreno,
                fatorRenda,
                valorEstatistico1,
                fatorRedeDrenagem,
                valorTotalDoImovel,
                valorEstatistico2),
            {
                text: 'Valor do Terreno Mínimo = Área do Terreno * VU * 0.85 = ' + "121" + ' * ' + "121" + ' * 0.85 = ' +  'R$' + "121",
                style: 'contas'
            },
            {
                text: 'Valor do Terreno Médio = Área do Terreno * VU = ' + "121" + ' * ' + "121" + ' = ' + 'R$' + "121",
                style: 'contas'
            },
            {
                text: 'Valor do Terreno Máximo = Área do Terreno * VU * 1.15 = ' + "121" + ' * ' +"121"+ ' * 1.15 = ' + 'R$' + "121",
                style: 'contas'
            },
            {
                text: [{ text: 'OBS:', bold: true }, 'A planta de valores do município possui um valor mínimo, médio e máximo do valor do m² para o terreno na face de quadra. Com isso, para o cálculo do valor do terreno também são calculados um valor mínimo, médio e máximo.'],
                margin: [0, 5, 0, 0],
                fontSize: 10
            },
            {
                text: '1.4.	Valores para o Terreno',
                style: 'subheader3'
            },
            {
                text: [{ text: 'Valor Mínimo do Terreno', bold: true }, ' = R$', "455344"],
                fontSize: 10
            },
            {
                text: [{ text: 'Valor Médio do Terreno', bold: true }, ' = R$', "455344"],
                fontSize: 10
            },
            {
                text: [{ text: 'Valor Máximo do Terreno', bold: true }, ' = R$', "455344"],
                fontSize: 10
            },
            {
                text: '2.	Cálculo do valor da Edificação (SINDUSCON-MA):',
                style: 'subheader2',
                margin: [0, 10, 0, 0]
            },
            {
                text: '2.1.	Fórmula',
                style: 'subheader3'
            },
            {
                text: 'VE = 0,20 * CUB * Area + 0,80 * CUB * D * Area',
                style: 'contas',
                fontSize: 10
            },
            {
                text: '2.2.	Parâmetros utilizados no cálculo',
                style: 'subheader3'
            },
            {
                text: [{ text: 'VE', bold: true }, ' = Valor da Edificação'],
                fontSize: 10
            },
            {
                text: [{ text: 'CUB', bold: true }, ' = Custo Unitário Básico (SINDUSCON-MA) => ' + "455344"],
                fontSize: 10
            },
            {
                text: [{ text: 'Tipologia', bold: true }, ' = Tipologia do imóvel para definição do CUB => ' + "455344"],
                fontSize: 10
            },
            {
                text: [{ text: 'Padrão de Construção', bold: true }, ' = Padrão de construção do imóvel => ' + "455344" ],
                fontSize: 10
            },
            {
                text: [{ text: 'D', bold: true }, ' = Depreciação do imóvel (Ross/Heidecke) => ' + "455344"],
                fontSize: 10
            },
            {
                text: [{ text: 'Idade do imóvel', bold: true }, ' = Idade do imóvel para cálculo depreciação => ' + "455344"],
                fontSize: 10
            },
            {
                text: [{ text: 'Estado de Conservação', bold: true }, ' = Estado de conservação do imóvel para definição da depreciação =>' + "455344"],
                fontSize: 10
            },
            {
                text: [{ text: 'Area', bold: true }, ' = O valor da área Construída da Edificação => ' + "455344"],
                fontSize: 10
            },
            {
                text: 'OBS: Os demais valores são constantes da fórmula pertencentes a este bloco.',
                margin: [0, 10, 0, 0],
                fontSize: 10
            },
            {
                text: '2.3.	Resolução do cálculo',
                style: 'subheader3'
            },
            {
                text: 'VE = 0,20 * ' + "455344" + ' * ' + "455344" + ' + 0,80 * ' + "455344" + ' * ' + "455344" + ' * ' + "455344" + ' = ' + 'R$' + "455344",
                style: 'contas'
            },
            {
                text: '2.4. Valor da Edificação',
                style: 'subheader3'
            },
            {
                text: [{ text: 'Valor da Edificação', bold: true }, ' = ', 'R$' + "455344"],
                fontSize: 10
            },
            {
                text: '3.	Valor TOTAL DO IMÓVEL',
                style: 'subheader2',
                margin: [0, 10, 0, 0]
            },
            {
                text: '3.1.	Fórmula',
                style: 'subheader3'
            },
            {
                text: 'Valor do imóvel = VT + VE',
                style: 'contas'
            },
            {
                text: '3.2.	Parâmetros utilizados no cálculo',
                style: 'subheader3'
            },
            {
                text: [{ text: 'VT', bold: true }, ' = Valor calculado para o Terreno'],
                fontSize: 10
            },
            {
                text: [{ text: 'VE', bold: true }, ' = Valor calculado para a Edificação'],
                fontSize: 10
            },
            {
                text: '3.3.	Resolução do cálculo',
                style: 'subheader3'
            },
            {
                text: 'Valor do imóvel (Mínimo) = VE + Valor Mínimo do Terreno = ' + "121" + ' + ' + "121" + ' = ' + 'R$' + "121",
                style: 'contas'
            },
            {
                text: 'Médio: VE + Valor Médio do Terreno = ' + "121"+ ' + ' + "121" + ' = ' + 'R$' + "121",
                style: 'contas'
            },
            {
                text: 'Máximo: VE + Valor Máximo do Terreno = ' + "121" + ' + ' +"121" + ' = ' + 'R$' + "121",
                style: 'contas'
            },
            {
                text: '3.4.	Valor Total do imóvel',
                style: 'subheader3'
            },
            {
                text: 'Valor do Imóvel:',
                bold: true
            },
            {
                text: [{ text: 'Mínimo', bold: true }, ' = R$', "121"],
                fontSize: 10
            },
            {
                text: [{ text: 'Médio', bold: true }, ' = R$',"121"],
                fontSize: 10
            },
            {
                text: [{ text: 'Máximo', bold: true }, ' = R$', "121"],
                fontSize: 10
            }
        ],
        styles: {
            header: {
                fontSize: 12,
                bold: true
            },
            subheader: {
                fontSize: 12,
                bold: true,
                margin: [0, 20, 0, 10]
            },
            subheader2: {
                fontSize: 12,
                bold: true,
            },
            subheader3: {
                fontSize: 12,
                bold: true,
                margin: [20, 10, 0, 5]
            },
            contas: {
                italics: true,
                fontSize: 10
            },
            conteudo: {
                fontSize: 10
            }
        }
    }


    return bloco;
}

function pdfObjectBlocosResumido(
    bloco,
    inscricao,
    valoresCalculados
) {
    let object = {
        content: [
            {
                image: image,
                width: 150,
                alignment: 'center',
                margin: [0, 0, 0, 15]
            },
            {
                text: 'Cálculo para o valor da 3ª Avaliação',
                style: 'header'
            },
            {
                text: [{ text: 'Inscrição Imobiliária: ', bold: true }, inscricao]
            },
            {
                text: ['A inscrição imobiliária informada faz parte do ', { text: bloco, bold: true }, ' da planta de valores.']
            },
            {
                text: 'RESUMO DO CÁLCULO',
                alignment: 'center',
                style: 'subheader'
            },
            {
                text: 'Valor do Terreno:',
                bold: true,
                fontSize: 10
            },
            {
                text: [{ text: 'Mínimo ', bold: true, fontSize: 10 }, '= R$', "121"]
            },
            {
                text: [{ text: 'Médio ', bold: true, fontSize: 10 }, '= R$',"121"]
            },
            {
                text: [{ text: 'Máximo ', bold: true, fontSize: 10 }, '= R$', "121"]
            },
            {
                text: [{ text: 'Valor da Edificação ', bold: true, fontSize: 10 }, '= R$',"121"],
                margin: [0, 10, 0, 10]
            },
            {
                text: 'Valor do Imóvel:',
                bold: true,
                fontSize: 10
            },
            {
                text: [{ text: 'Mínimo ', bold: true, fontSize: 10 }, '= R$', "121"]
            },
            {
                text: [{ text: 'Médio ', bold: true, fontSize: 10 }, '= R$', "121"]
            },
            {
                text: [{ text: 'Máximo ', bold: true, fontSize: 10 }, '= R$',"121"]
            }
        ],
        styles: {
            header: {
                fontSize: 12,
                bold: true
            },
            subheader: {
                fontSize: 12,
                bold: true,
                margin: [0, 20, 0, 10]
            },
            subheader2: {
                fontSize: 12,
                bold: true,
            },
            subheader3: {
                fontSize: 12,
                bold: true,
                margin: [20, 10, 0, 5]
            },
            contas: {
                italics: true,
                fontSize: 10
            }
        }
    }
    return object
}

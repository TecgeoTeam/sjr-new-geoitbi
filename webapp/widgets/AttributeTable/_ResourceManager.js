  ...
  return declare(null, {
        ...
        .then(lang.hitch(this, function(layerInfos) {
          ...
          if (updateConfig) {
            if (this.config.layerInfos.length === 0) {
              ...
              let configLayerInfos = attrUtils.getConfigInfosFromLayerInfos(layerInfos);
              ...
              this.config.layerInfos = array.filter(
                ...
                lang.hitch(this, function(layerInfo) {
                  let mLayerInfo = this._getLayerInfoById(layerInfo.id);
    ...
    addConfigInfo: function(newLayerInfo) {
      if (!this._getConfigInfoById(newLayerInfo.id)) {
        let info = attrUtils.getConfigInfoFromLayerInfo(newLayerInfo);
    ...
    removeLayerInfo: function(infoId) {
      let _clayerInfo = this._getLayerInfoById(infoId);
      let pos = this._layerInfosFromMap.indexOf(_clayerInfo);
      this._layerInfosFromMap.splice(pos, 1);
    },

    removeConfigInfo: function(infoId) {
      if (lang.getObject('config.layerInfos', false, this)) {
        let len = this.config.layerInfos.length;
        for (let i = 0; i < len; i++) {
    ...
    getQueryTable: function(tabId, enabledMatchingMap, hideExportButton) {
      ...
      if (!this.featureTableSet[tabId]) {
        this._getQueryTableInfo(tabId).then(lang.hitch(this, function(queryTableInfo) {
          ...
          let activeLayerInfo = queryTableInfo.layerInfo;
          let layerObject = queryTableInfo.layerObject;
          let tableInfo = queryTableInfo.tableInfo;
          ...
          if (lang.getObject('isSupportQuery', false, tableInfo)) {
            let configInfo = this._getConfigInfoById(tabId);
            if (!configInfo) {
              this.addConfigInfo(activeLayerInfo);
              configInfo = this._getConfigInfoById(tabId);
            }
            let configFields = lang.getObject('layer.fields', false, configInfo);
            let layerFields = layerObject && layerObject.fields;
    ...
    getRelationTable: function(originalInfoId, key, enabledMatchingMap, hideExportButton) {
      let def = new Deferred();
      let currentShip = this.relationshipsSet[key];
      this._activeRelationshipKey = key;

      if (currentShip) {
        let originalInfo = this._getLayerInfoById(originalInfoId);
        let layerInfoId = lang.getObject('shipInfo.id', false, currentShip);
    ...
    getRelationshipsByInfoId: function(id) {
      let ships = [];
      for (let p in this.relationshipsSet) {
        let ship = this.relationshipsSet[p];
    ...
    _getLayerInfoById: function(layerId) {
      for (let i = 0, len = this._layerInfosFromMap.length; i < len; i++) {
    ...
    _getConfigInfoById: function(id) {
      ...
      for (let i = 0, len = this.config.layerInfos.length; i < len; i++) {
        let configInfo = this.config.layerInfos[i];
    ...
    _getQueryTableInfo: function(tabId) {
      let def = new Deferred();
      let activeLayerInfo = this._getLayerInfoById(tabId);
      ...
      if (!activeLayerInfo) {
        ...
        let defs = [];
        let hasUrl = activeLayerInfo.getUrl();
        ...
        all(defs).then(lang.hitch(this, function(results) {
          ...
          let tableInfo = results[1];
          let relatedTableInfos = hasUrl ? results[2] : [];
    ...
    _collectRelationShips: function(layerInfo, layerObject, relatedTableInfos) {
      ...
      if (ships && ships.length > 0 && layerObject && layerObject.url) {
        let layerUrl = layerObject.url;
        let parts = layerUrl.split('/');

        array.forEach(ships, lang.hitch(this, function(ship) {
          parts[parts.length - 1] = ship.relatedTableId;
          let relationUrl = parts.join('/');

          let tableInfos = array.filter(relatedTableInfos, lang.hitch(this, function(tableInfo) {
            let tableInfoUrl = tableInfo.getUrl();
          ...
          let relKey = layerInfo.id + '_' + ship.name + '_' + ship.id;
          ship._relKey = relKey;
    ...
    _clipValidFields: function(sFields, rFields) {
      ...
      let validFields = [];
      for (let i = 0, len = sFields.length; i < len; i++) {
        let sf = sFields[i];
        for (let j = 0, len2 = rFields.length; j < len2; j++) {
          let rf = rFields[j];
          if (rf.name === sf.name) {
            validFields.push(sf);
            break;
      ...
      if (validFields.length === 0 && sFields.length > 0) {
        let fieldInfos = lang.clone(rFields);

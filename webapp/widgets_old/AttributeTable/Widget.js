  ...
  let clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    _createBarUI: function() {
      ...
      if (!this.arrowDivHeight) {
        let arrowDivBox = html.getMarginBox(this.arrowDiv);
    ...
    destroy: function() {
      ...
      if (this.layerTabPages && this.layerTabPages.length > 0) {
        ...
        for (i = 0; i < len; i++) {
          let paneId = this.layerTabPages[i].paneId;
          let table = lang.getObject('_resourceManager.featureTableSet.' + paneId, false, this);
          if (table) {
            let filterObj = table.getFilterObj();
    ...
    _changeHeight: function(h) {
      ...
      if (h !== 0) {
        let minOpenHeight = this.arrowDivHeight;
    ...
    setPosition: function(position) {
      ...
      if (!this._resourceManager.isEmpty()) {
        setTimeout(lang.hitch(this, function() {
          let ngHeight = this._getGridTopSectionHeight();
          let domHeight = html.getStyle(this.domNode, 'height');
          if (ngHeight > 0) {
    ...
    _startQueryOnLayerTab: function(tabId, featureSet) {
      let layerInfo = this._resourceManager.getLayerInfoById(tabId);
      let tabPage = this.getExistLayerTabPage(tabId);
      ...
      if (layerInfo && tabPage) {
        ...
        this._resourceManager.getQueryTable(
          ...
          if (result.isSupportQuery) {
            ...
            let tip = html.toDom('<div>' + this.nls.unsupportQueryWarning + '</div>');
    ...
    _startQueryOnRelationTab: function(infoId, relationShipKey, selectedIds, originalInfoId) {
      let originalInfo = this._resourceManager.getLayerInfoById(originalInfoId);
      let tabPage = this.getExistLayerTabPage(infoId);
    ...
    tabChanged: function() {
      if (this.tabContainer && this.tabContainer.selectedChildWidget) {
        ...
        let params = this.tabContainer.selectedChildWidget.params; // ContentPage.params

        let layerType = params.layerType;
        let infoId = params.paneId;
        let relKey = params.relKey;
        ...
        if (layerType === this._layerTypes.FEATURELAYER &&
          ...
          this.setActiveTable(null);
          let selectIds = params.oids;
          let originalInfoId = params.originalInfoId;
          this._activeLayerInfoId = infoId;
    ...
    _onDragEnd: function() {
      ...
      let h = this._dragingHandlers.pop();
      while (h) {
        h.remove();
      ...
      if (that._activeTable) {
        ...
        that._activeTableHandles.push(on(that._activeTable,
          'show-all-records', function(evt) {
            let page = that.getExistLayerTabPage(evt.layerInfoId);
    ...
    _unbindActiveTableEvents: function() {
      let that = this;
      let handlers = that._activeTableHandles;
    ...
    setActiveTable: function(table, options) {
      ...
      if (table) {
        ...
        if (!this._activeTable.tableCreated ||
          ...
          let validFeatureSet = lang.getObject('featureSet.features.length', false, options);
          if (options.layer && options.selectedIds) {
            this._activeTable.queryRecordsByRelationship(options);
          } else if (validFeatureSet) {
            let primaryId = options.featureSet.displayFieldName;
            let featureIds = array.map(options.featureSet.features, function(f) {
    ...
    initDiv: function() {
      ...
      let configInfos = this._resourceManager.getConfigInfos();
      let len = configInfos.length;
      for (let j = 0; j < len; j++) {
        let configInfo = configInfos[j];
        if (configInfo.show) {
          let json = lang.clone(configInfo);
          let paneJson = {};
          ...
          let cp = new ContentPane(paneJson);
    ...
    getLayerInfoLabel: function(layerInfo) {
      let label = layerInfo.name || layerInfo.title;
      return label;
    },
    ...
    _getGridTopSectionHeight: function() {
      let tabPageWrapper = domQuery('.dijitTabPaneWrapper', this.domNode)[0];
      if (tabPageWrapper) {
        let widgetTop = html.position(this.domNode).y;
        let tabPaneTop = html.position(tabPageWrapper).y;
    ...
    _getLayerInfoByIdFromConfigJSON: function(id) {
      let configedInfos = array.filter(this.config.layerInfos, function(linfo) {
        return linfo.id === id;
      });
    ...
    _showRelatedRecords: function() {
      let activeTable = this._activeTable;
      if (activeTable) {
        let layerInfo = activeTable.layerInfo;
        if (layerInfo && layerInfo.layerObject) {
          let _layer = layerInfo.layerObject;
          let ships = _layer.relationships;
          let objIds = activeTable.getSelectedRows();
    ...
    addNewLayerTab: function(infoId, featureSet) {
      let layerInfo = this._resourceManager.getLayerInfoById(infoId);
      if (!layerInfo) {
        return;
      }
      let page = this.getExistLayerTabPage(infoId);
      let json = {};
    ...
    addNewRelationTab: function(oids, relationShip, originalInfoId) {
      let lInfo = relationShip && relationShip.shipInfo;
      if (!lInfo) {
        return;
      }
      let page = this.getExistLayerTabPage(relationShip.shipInfo.id);

      let json = {};
      json.oids = oids;
      let paneTitle = lInfo.title || lInfo.name || relationShip.name;
      ...
      if (params && params.target === 'AttributeTable') {
        ...
        if (!this.showing) {
          this._openTable().then(lang.hitch(this, function() {
            let isInResources = !!this._resourceManager.getLayerInfoById(params.layer.id);
            if (!isInResources) {
    ...
    _addLayerToTable: function(params) {
      ...
      let layerInfo = this._resourceManager
        .getLayerInfoById(
    ...
    getExistLayerTabPage: function(paneId) {
      let len = this.layerTabPages.length;
      for (let i = 0; i < len; i++) {
    ...
    layerTabPageClose: function(paneId, isRemoveChild) {
      let len = this.layerTabPages.length;
      let activeId = lang.getObject('_activeTable.layerInfo.id', false, this);
      if (activeId === paneId) {
        this.setActiveTable(null);
      }
      for (let i = 0; i < len; i++) {
        if (this.layerTabPages[i] && this.layerTabPages[i].paneId === paneId) {
          ...
          if (len === 1) {
            ...
            let layerIndex = len - 2; // show last contentpane
    ...
    _processRelatedRecordsFromPopup: function(layerInfo, featureIds) {
      if (layerInfo) {
        ...
        all(defs).then(lang.hitch(this, function(results) {
          if (results) {
            let layerObject = results[0];
            let relatedTableInfos = results[1];
            this._resourceManager.collectRelationShips(layerInfo, relatedTableInfos);
            let ships = layerObject.relationships;
            for (let i = 0, len = ships.length; i < len; i++) {
    ...
    showRelatedRecordsFromPopup: function(layerInfo, featureIds) {
      if (!this.showing) {
        this._openTable().then(lang.hitch(this, function() {
          let isInResources = !!this._resourceManager.getLayerInfoById(layerInfo.id);
          if (!isInResources) {
            this._resourceManager.updateLayerInfoResources(false)
              .then(lang.hitch(this, function() {

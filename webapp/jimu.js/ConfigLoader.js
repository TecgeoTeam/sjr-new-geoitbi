  ...
  function (declare, lang, array, html, dojoConfig, cookie,
    ...
    ********************************************************/
    ...
    loadConfig: function () {
      ...
      return this._tryLoadConfig().then(lang.hitch(this, async function(appConfig) {

        //console.log(appConfig)
        //let dadosUsuario = JSON.parse(localStorage["ngStorage-dados"])
        //console.log(dadosUsuario.usuario)
        
        if(localStorage["ngStorage-insc"]){
          for (let i = 0; i < appConfig.widgetPool.widgets.length; i++) {
            if (appConfig.widgetPool.widgets[i].openAtStart === false) {
              appConfig.widgetPool.widgets[i].openAtStart = true
            }
          }
        }
        ...
        document.addEventListener('click',function(e){
          if(e.target && e.target.id == 'closeGeoITBI'){
              var r = confirm("Você deseja mesmo sair do GeoITBI?");
              if (r == true) {
                txt = "Saindo...";
                //do logout
                fetch("https://sigribamar.com.br/geoitbi_login/geoportal/logout", {
                  method: 'POST',
                ...
                .then(
                  (response) => {
                    ...
                    //limpar a sessão
                    localStorage.clear();
                    window.top.location.href = "http://geoitbisjr.tecgeobr.com.br";
        ...
        //Usuário está logado
        let logado = await fetch("https://sigribamar.com.br/geoitbi_login/geoportal/logged", {
          method: 'POST',
          body: JSON.stringify({email:localStorage.getItem('email'), salt:localStorage.getItem('salt')}),
          headers: {
            "Content-Type": "application/json"
        ...
        if(dadosUsuario && dadosUsuario.usuario.logged){
          //Remove widget without permission
          //console.log(appConfig.widgetOnScreen.widgets);
          var widgetsOnScreen = appConfig.widgetOnScreen.widgets;
          var widgetsOnScreenWithPermission = [];
          var initialLeftPosition = 55;

          for (wgt in widgetsOnScreen){
            if (widgetsOnScreen[wgt].name === "HeaderController"){
              widgetsOnScreenWithPermission.push(widgetsOnScreen[wgt]);
            }
            if (widgetsOnScreen[wgt].name === "ZoomSlider"){
              widgetsOnScreenWithPermission.push(widgetsOnScreen[wgt]);
            }
            if (widgetsOnScreen[wgt].name === "HomeButton"){
              widgetsOnScreenWithPermission.push(widgetsOnScreen[wgt]);
            }
          }

          for (var j = 0; j < widgetsOnScreen.length; j++) {
            for (var g = 0; g < dadosUsuario.usuario.acls.length; g++) {
              var groupResource = dadosUsuario.usuario.acls[g]; 
              if(widgetsOnScreen[j].name === groupResource.codigo_recurso){
                if(widgetsOnScreen[j].position && (widgetsOnScreen[j].position.top===45)){
                  widgetsOnScreen[j].position.left=initialLeftPosition;
                  initialLeftPosition+=50;
                }

                widgetsOnScreenWithPermission.push(widgetsOnScreen[j]);
                break;
              }
            }

          }
          appConfig.widgetOnScreen.widgets = widgetsOnScreenWithPermission;

          var widgetsPool = appConfig.widgetPool.widgets;
          var widgetsPoolWithPermission = [];
          console.log(appConfig.widgetPool.widgets);

          for (wgtPool in widgetsPool){
            if (widgetsPool[wgtPool].name === "About"){
              widgetsPoolWithPermission.push(widgetsPool[wgtPool]);
            }
          }

          for (var j = 0; j < widgetsPool.length; j++) {
            for (var g = 0; g < dadosUsuario.usuario.acls.length; g++) {
              var groupResource = dadosUsuario.usuario.acls[g];
              if(widgetsPool[j].name === groupResource.codigo_recurso){

                if(widgetsPool[j].position && (widgetsPool[j].position.top===45)){
                  widgetsPool[j].position.left=initialLeftPosition;
                  initialLeftPosition+=50;
                }

                widgetsPoolWithPermission.push(widgetsPool[j]);
                break;
              }
            }

          }
          appConfig.widgetPool.widgets = widgetsPoolWithPermission;
        } else {
        ...
        /*if(dadosUsuario.usuario.logged){
          //Remove widget without permission
          console.log(appConfig.widgetOnScreen.widgets);
          var widgetsOnScreen = appConfig.widgetOnScreen.widgets;
          var widgetsOnScreenWithPermission = [];
          ...
          for (var j = 0; j < widgetsOnScreen.length; j++) {
            for (var g = 0; g < dadosUsuario.usuario.grupos[0].recursos.length; g++) {
              var groupResource = dadosUsuario.usuario.grupos[0].recursos[g]; 
              if(widgetsOnScreen[j].name === groupResource.nome){
                if(widgetsOnScreen[j].position && (widgetsOnScreen[j].position.top===45)){
                  widgetsOnScreen[j].position.left=initialLeftPosition;
                  initialLeftPosition+=50;
                }
          ...
          for (var j = 0; j < widgetsPool.length; j++) {
            for (var g = 0; g < dadosUsuario.usuario.grupos["0"].recursos.length; g++) {
              var groupResource = dadosUsuario.usuario.grupos["0"].recursos[g];
              if(widgetsPool[j].name === groupResource.nome){

    ...
    return declare([BaseWidget], {
      ...
      searchLote: function(wkid, callback) {
        ...
        let shapeFieldName;
          ...
          layerFieldName = 'sjr_cadastro.SDE.Lote.GEOC_RENU';
      ...
      saveTerceiraAvaliacao: function() {
        ...
        if (Number(terceiraAvaliacaoValue) > Number(valorMinimo)) {
          ...
          $.ajax({
            url: 'http://zeus:1010/terceiraavaliacao',
            type: 'post',
            data: {
              geocodigo: geocode,
...
function factoryViewDataSTM(title, tbody, dados) {
    ...
    let aliases;
    if (selfCalculo.config.fieldAliases){
      aliases = selfCalculo.config.fieldAliases;
    }
    for (value in dados){
        let tr = document.createElement('tr');
        let tdLabel = document.createElement('td');
        ...
        let ted;
        if (dados[value] !== 'null' && dados[value] !== null && dados[value] !== undefined) {
